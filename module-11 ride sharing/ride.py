...
class RideSharing:
    ...
    def __str__(self):
        return f"Company Name: {self.company_name}, riders: {len(self.riders)} drivers: {len(self.drivers)}"
...
class Ride:
    ...
    def __repr__(self):
        return f"Ride details. Started {self.start_location} ot {self.end_location}"
...
class RideMatching:
    ...
    def find_driver(self, ride_request, vehicle_type):
        if len(self.available_drivers) > 0:
            ...
            if vehicle_type == "car":
                vehicle = Car("car", "LIC123XYU", 300)
            elif vehicle_type == "bike":
                vehicle = Bike("bike", "LIC432VC", 500)
